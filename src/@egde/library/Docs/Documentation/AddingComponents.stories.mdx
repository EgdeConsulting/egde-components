...
import { Root, DocsPageHeader, DocsPageBodyComponents } from '@egde/library';
...
Create two new sub-folders, one inside of `src/@egde/components/.../_componentName_` and the other in `src/@egde/library/Components/.../_componentName_`
where `...` corresponds to the category under which the new component should be listed, and `_componentName_` is replace with
...
Inside of the component-folder located at `src/@egde/components/...` create and export a new React component in a `.tsx` file.
Afterwards export the React component by adding a `index.ts` file to the component-folder where you:
...
the component-folder located at `src/@egde/library/Components/...`, and start editing the file by including the following
three import-statements were `_componentName_` needs to be replaced:

```jsx
import { Meta, Story, Canvas } from '@storybook/addon-docs';
import { LibPath, DocsPageHeader, DocsPageBodyComponents } from '@egde/library';
import { _componentName_ } from '@egde/components';
```
...
> which is located under `src/@egde/library/Docs` and add a new enum for your category under `LibPath`.
...
`_componentName_.stories.mdx`. After that add the following import-statement:
...
if the specific property value is being used inside the `args` prop of the Meta-tag.
