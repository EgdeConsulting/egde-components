...
Create a new sub-folder under `src/egde-library/Templates/_templateName_` and inside there create a file
called `_templateName_.tsx`. This file is going to contain the source code of the new template. Start of
editing the file by adding the following two import-statements:
...
> use `from 'egde-components'` or `from 'egde-icons'` when importing components and icons.
...
> to learn more about `Meta`, `Story` and `Canvas`.
