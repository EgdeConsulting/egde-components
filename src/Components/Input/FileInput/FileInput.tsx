...
export const FileInput = (props: FileUploadProps): ReactElement => {
  const { label, labelTextMargin, invalidText, captionText, margin, padding, width, minWidth, size, variant } = props;

  const fileInputStyles = useMultiStyleConfig('FileInput', { size, variant });

  const [totalUploadSizeReached, setTotalUploadSizeReached] = useState<boolean>(false);
  const [fileUploadSizeReached, setFileUploadSizeReached] = useState<boolean>(false);
  ...
  const getValidationText = (isDragActive: boolean) => {
    return (
      <Box
        ...
        onMouseEnter={() => variant !== 'disabled' ?? setHoverUploadBox(true)}
        onMouseLeave={() => variant !== 'disabled' ?? setHoverUploadBox(false)}
      >
        <Center>
          {!isDragActive && !totalUploadSizeReached && !fileUploadSizeReached
            ? props.uploadLabel ?? 'Dra og slipp filer her, eller klikk for å søke etter filer.'
            : isDragActive
            ? props.dropLabel ?? 'Slipp filer'
            : totalUploadSizeReached
            ? props.maxUploadSizeLabel ??
              'Maks opplasting er ' + (props.maxUploadSize / 1024 / 1024).toLocaleString() + ' Mb'
            : props.maxFileSizeLabel ??
              'Maksstørrelse pr fil er ' + (props.maxFileSize / 1024 / 1024).toLocaleString() + ' Mb'}
        </Center>
        {props.showIcon && (
  ...
  return (
    ...
    >
      <Center>
        <Box width={size === 'sm' ? FileInputStyles.sizes.sm.width : FileInputStyles.sizes.md.width}>
          <Dropzone
          ...
          >
            {({ getRootProps, getInputProps, isDragActive }) => {
              return (
                <div {...getRootProps()}>
                  <input {...getInputProps()} />
                  <Center>{getValidationText(isDragActive)}</Center>
                </div>
              );
            }}
          </Dropzone>
